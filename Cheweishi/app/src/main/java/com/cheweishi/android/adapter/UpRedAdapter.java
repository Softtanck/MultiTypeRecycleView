...
public class UpRedAdapter extends BaseAdapter {

    private Context mContext;
    private List<PurseIntegralResponse.MsgBean> mList;
    private LayoutInflater mLayoutInflater;

    public UpRedAdapter(Context context, List<PurseIntegralResponse.MsgBean> list) {
        mContext = context;
        mList = list;
    ...
    public View getView(int position, View convertView, ViewGroup arg2) {
        ...
        if (mList != null && mList.size() > position) {
            ...
            if (rule.equals("INCOME")) {
                viewHolder.integralTextView.setText("+" + mList.get(position).getRedPacket() + "元");
