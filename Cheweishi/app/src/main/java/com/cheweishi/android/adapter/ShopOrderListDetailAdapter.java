 ...
 */
public class ShopOrderListDetailAdapter extends BaseAdapter {

    private Context context;
    private List<ShopOrderListResponse.MsgBean.OrderItemBean> list;

    public ShopOrderListDetailAdapter(Context context, List<ShopOrderListResponse.MsgBean.OrderItemBean> list) {
        this.context = context;
        this.list = list;
    }

    public void setData(List<ShopOrderListResponse.MsgBean.OrderItemBean> list) {
        this.list = list;
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (StringUtil.isEmpty(list.get(position).getThumbnail()))
            holder.icon.setScaleType(ImageView.ScaleType.FIT_XY);
        XUtilsImageLoader.getHomeAdvImg(context, R.drawable.udesk_defualt_failure, holder.icon, list.get(position).getThumbnail());
        holder.name.setText(list.get(position).getName());
        holder.number.setText("x" + list.get(position).getQuantity());
