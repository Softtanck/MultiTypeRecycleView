...
public class MySurfaceView extends SurfaceView implements
	...
	class MyThread implements Runnable {
		public void run() {
			...
			Canvas canvas = null;
			if (getWidth() > 0) {
				Bitmap bak = Bitmap.createBitmap(getWidth(), getHeight(),
				...
				while (rotate < 100) {
					...
					try {
						...
						mPaint.setColor(Color.rgb(random.nextInt(255),
								random.nextInt(255), random.nextInt(255)));
						mPaint.setStrokeWidth(2);/* ����paint������� */
						Path mPath = new Path();
						...
						if (before < 17) {
							Thread.sleep(17 - before); // ����
						}

					} catch (InterruptedException e) {
						e.printStackTrace();
					} finally {
						holder.unlockCanvasAndPost(canvas); // ����canvas���ύ���õ�ͼ��
					}
				}
			}
		}
